...
@DisplayName("When handling  task title changed event")
public class WhenHandlingTaskTitleChangedEventTestTest extends ProjectionTest<TaskTitleChanged> {
    @Captor
    ...
    @Override
    protected TaskTitleChanged when() {
        return new TaskTitleChanged(taksId, "Title 1", 2);
    }
    ...
    @Test
    @DisplayName("Test task projection has same title changed as event")
    void testProjectionIsSavedWithSameTitle() {
